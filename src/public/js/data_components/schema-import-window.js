class SchemaImportWindow extends ELMNT
{
    ...
    _onFileUpload(){
        ...
        reader.onload = (e)=>{
            ...
            try{
                ...
                this.$el('results').innerText = `Schema Ready for Import as ${this.pk.split('$')[0]}`;
            } catch (e){
    ...
    async _onSubmit(){
        try{
            const results = await Global.post(
                'data',
                { action: 'import' },
                {pk: this.pk + '/dataset', sk: 'dataset', schema: this.#data }
            );
            if (results === true){
                this.dispatchEvent(new CustomEvent('complete',{detail:this.data}));
                this.dataset.state = 'upload-complete';
                this.$el('results').innerText = 'Upload Complete!';
            } else {
    ...
    // --- Static Defs   --- //
    static Tag = 'schema-import-window';

    static Html = (
        `<div class="title"><span>SCHEMA IMPORT WIZARD</span><button id="cancel">✕</button></div>
        <div class="controls">
            <button id="upload" class="dyn ready-for-file">Upload File</button>
            <button id="submit" class="dyn ready-for-import">Import Schema</button>
        </div>
...
ELMNT.define(SchemaImportWindow);
